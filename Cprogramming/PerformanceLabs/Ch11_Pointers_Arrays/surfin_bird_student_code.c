...
char * find_the_word(char * sentence_ptr, char * searchWord_ptr, int * errorCode_ptr)
{
	//if the sentence is empty return an error
	if (!sentence_ptr)
	{
		*errorCode_ptr = ERROR_NULL_SENTENCE_POINTER;
		return NULL;
	}
	//if the searchWord ptr is empty return an error
	...
	//if there is no error code pointer then just return null - since theres nowhere to store the proper error code for this...
	...
	{
		int iterator = 0;
		char *firstOccurence = 0;
		//this loop walks through each letter in the sentence and then loops through if a character matches the first character in the search string
		while (*(sentence_ptr+ iterator) != '\0')
		{
			int secondIterator = 0; //reset the second iterator if the word was not found (IE if the word was similar but not exact)

			//this loop iterates through each letter in the search word and sees if it equals the equivalent spot in the sentence.
			while (*(sentence_ptr + iterator + secondIterator) == *(searchWord_ptr + secondIterator))
			{
				//if the next character in the search string is null that means we have completely checked the whole string and it does exist in the sentence!!!
				if (*(searchWord_ptr + secondIterator + 1) == '\0')
				{
					//if we found the word in the sentence then make the first occurence the original sentence + the iterator aka the first letter that matched
					firstOccurence = (sentence_ptr + iterator);
				}
				secondIterator++;
			}
			//if there is a value in first occurence then we have found the word and can return success and the value. 
			if (firstOccurence != '\0')
			{
		...
		//in the case that the search is not successful then make the error code the search was not found and then return NULL
